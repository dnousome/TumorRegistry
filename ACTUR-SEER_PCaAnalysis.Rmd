...
labelsdt=list(year_cat="Year of Diagnosis",age="Age at diagonsis",age_cat5="Age Categories",race_new="Race",
              stage_final1="Tumor Stage",tumor_grade1="Tumor Grade",age_cat1="Age Categories",
...
dt_final=read_rds("~/Documents/G/Work_Bench/Epidemiology/Projects/MCC_CPDR SEERACTUR 2020/Data/mcc_actur.rds") %>%
  ...
  mutate(Dxyr_PSA=factor(Dxyr_PSA,levels=c("0","1","2","3","8","9"),
                         labels=c("2010-2013 & PSA 0-<4","2010-2013 & PSA 4-<10","2010-2013 & PSA 10-<20","2010-2013 & PSA \u226520",
                                  "2010-2013 & PSA Missing","All Other years & PSA Missing"))) %>%
...
## Summary tables
```{r tables,echo=F}
tab.noby <- tableby(dataset~ age+age_cat1+year_cat+race_new+Hispanic+stage_final1+tumor_grade1+PSA+PSA_category+PSA_cat_rem+
...
summary(tab.noby,labelTranslations=labelsdt,digits=1) %>% 
  ...
  add_footnote(c("PSA values and Gleason scores were restricted for data only from 2010-2013",
                 "Missing Tumor Grade was replaced by Gleason score when available"),notation = "none")
...
fit <- survfit(Surv(fiveyear_FU, fiveyear_death) ~ dataset,
               data = dt_surv)
ggsurvplot(fit, data = dt_surv, risk.table = 'nrisk_cumevents', conf.int = T,censor=F,pval=T,fontsize=4,tables.y.text=F, legend.labs = c("ACTUR", "SEER"),xlab="Time (Months)",
           title="Five Year Survival Between ACTUR/SEER")


dt_surv_t3=dt_surv %>% filter(stage_final==3)
fit <- survfit(Surv(fiveyear_FU, fiveyear_death) ~ dataset,
               data = dt_surv_t3)
ggsurvplot(fit, data = dt_surv_t3, risk.table = 'nrisk_cumevents', conf.int = T,censor=F,pval=T,fontsize=4,tables.y.text=F, legend.labs = c("ACTUR", "SEER"),xlab="Time (Months)",
                    title="Five Year Survival Between ACTUR/SEER (T3 Stage only)")

dt_surv_t4=dt_surv %>% filter(stage_final==4)
fit <- survfit(Surv(fiveyear_FU, fiveyear_death) ~ dataset,
               data = dt_surv_t4)
ggsurvplot(fit, data = dt_surv_t4, risk.table = 'nrisk_cumevents', conf.int = T,censor=F,pval=T,fontsize=4,tables.y.text=F, legend.labs = c("ACTUR", "SEER"),xlab="Time (Months)",
                    title="Five Year Survival Between the ACTUR/SEER (T4 Stage only)")
```
...
labels_coxph=c(labelsdt,
          ...
          `as.factor(as.numeric(Dxyr_PSA))2`="PSA 4-<10 vs 0-<4",
          `as.factor(as.numeric(Dxyr_PSA))3`="PSA 10-<20 vs 0-<4",
          `as.factor(as.numeric(Dxyr_PSA))4`="PSA \u226520 vs 0-<4",
          `as.factor(as.numeric(Dxyr_PSA))5`="PSA Missing in 2010-2013 vs PSA 0-<4",
          `as.factor(as.numeric(Dxyr_PSA))6`="All Other years PSA Missing vs PSA 0-<4",

           `tumor_grade_gleason2`="Grade 2 vs 1",
           `tumor_grade_gleason3`="Grade 3 vs 1",
                      `tumor_grade1Unknown/Missing`="Unknown vs 1",
...
summary(modelsum(Surv(fiveyear_FU, fiveyear_death) ~dataset,adjust=~stage_final1+tumor_grade_gleason+as.factor(as.numeric(Dxyr_PSA))+
                   Surgery+radiation_new+age+as.factor(hispanic)+
                 
                    strata(pair),
                 data=dt_surv,family="survival"),
       labelTranslations=labels_coxph,digits=2,digits.p=3,digits.ratio=2) %>%
  kable(format="markdown") %>%
  add_footnote(c("Missing Tumor Grade was replaced by Gleason score when available","PSA available only from 2010-2013"),notation = "none")



fit1=coxph(Surv(fiveyear_FU, fiveyear_death) ~dataset+stage_final1+tumor_grade_gleason+as.factor(as.numeric(Dxyr_PSA))+
                   Surgery+radiation_new+age+as.factor(hispanic)+
...
summary(modelsum(Surv(fiveyear_FU, fiveyear_death) ~dataset,adjust=~tumor_grade_gleason+as.factor(as.numeric(Dxyr_PSA))+
...
summary(modelsum(Surv(fiveyear_FU, fiveyear_death) ~dataset,adjust=~stage_final1++as.factor(as.numeric(Dxyr_PSA))+
                   Surgery+radiation_new+age+as.factor(hispanic)+strata(pair),
                 data=surv1,family="survival",show.adjust=F),
       labelTranslations=labels_coxph,digits=2,digits.p=3,digits.ratio=2) %>%
  kable(format="markdown",caption="Grade 1")
